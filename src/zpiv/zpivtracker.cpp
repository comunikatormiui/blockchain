// Copyright (c) 2018-2020 The YODA developers
...
CzYODATracker::CzYODATracker(std::string strWalletFile)
{
...
CzYODATracker::~CzYODATracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzYODATracker::Init()
{
...
bool CzYODATracker::Archive(CMintMeta& meta)
{
...
bool CzYODATracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzYODATracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzYODATracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzYODATracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzYODATracker::GetSerialHashes()
{
...
CAmount CzYODATracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzYODATracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzYODATracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzYODATracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzYODATracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzYODATracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzYODATracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzYODATracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzYODATracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzYODATracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzYODATracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzYODAWallet* zYODAWallet)
{
    bool iszYODAWalletInitialized = (NULL != zYODAWallet);
    CMintMeta meta;
    ...
    if (! iszYODAWalletInitialized)
        zYODAWallet = new CzYODAWallet(strWalletFile);
    meta.isSeedCorrect = zYODAWallet->CheckSeed(dMint);
    if (! iszYODAWalletInitialized)
        delete zYODAWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzYODATracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzYODATracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzYODATracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzYODATracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzYODATracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzYODATracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzYODAWallet* zYODAWallet = new CzYODAWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zYODAWallet);
        }
        delete zYODAWallet;
