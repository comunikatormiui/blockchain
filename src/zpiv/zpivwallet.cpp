...
CzYODAWallet::CzYODAWallet(std::string strWalletFile)
{
        ...
        if (walletdb.ReadZPIVSeed_deprecated(seed)) {
            ...
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZPIVSeed_deprecated()) {
                    LogPrintf("%s: Updated zYODA seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzYODAWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
...
void CzYODAWallet::Lock()
{
    seedMaster = 0;
}

void CzYODAWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzYODAWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
...
bool CzYODAWallet::LoadMintPoolFromDB()
{
...
void CzYODAWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzYODAWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzYODAWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzYODAWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
...
void CzYODAWallet::SeedToZPIV(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzYODAWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzYODAWallet::GenerateDeterministicZPIV(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzYODAWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzYODAWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzYODAWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
