...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::YODA);
    ...
    if (settings.contains("nAnonymizeYodaAmount") || reset)
        SoftSetArg("-anonymizeyodaamount", settings.value("nAnonymizeYodaAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in yoda.cpp during shotdown to also remove the window positions
