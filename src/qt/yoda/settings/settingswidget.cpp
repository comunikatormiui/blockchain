...
#include "qt/yoda/settings/forms/ui_settingswidget.h"
...
SettingsWidget::SettingsWidget(YODAGUI* parent) :
    PWidget(parent),
    ui(new Ui::SettingsWidget)
{
    ...
    ui->pushButtonTools2->setVisible(false);
...
void SettingsWidget::onSaveOptionsClicked(){
    if(mapper->submit()) {
        ...
        if (this->clientModel->getOptionsModel()->isRestartRequired()) {
            bool fAcceptRestart = openStandardDialog(tr("Restart required"), tr("Your wallet needs to be restarted to apply the changes"), tr("Restart Now"), tr("Restart Later"));
