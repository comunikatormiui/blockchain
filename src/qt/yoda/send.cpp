...
#include "qt/yoda/send.h"
#include "qt/yoda/forms/ui_send.h"
#include "qt/yoda/addnewcontactdialog.h"
#include "qt/yoda/qtutils.h"
#include "qt/yoda/sendchangeaddressdialog.h"
#include "qt/yoda/optionbutton.h"
#include "qt/yoda/sendconfirmdialog.h"
#include "qt/yoda/myaddressrow.h"
#include "qt/yoda/guitransactionsutils.h"
...
SendWidget::SendWidget(YODAGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("YODA");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zYODA");
    setCssProperty(ui->pushRight, "btn-check-right");

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (YODA) or private coins (zYODA)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("Enter a YODA address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 YODA");
...
void SendWidget::refreshView(){
    ...
    if(ui->pushLeft->isChecked()){
        btnText = tr("Send YODA");
        ui->pushButtonAddRecipient->setVisible(true);
    }else{
        btnText = tr("Send zYODA");
...
void SendWidget::onSendClicked(){
    ...
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendPiv ? AskPassphraseDialog::Context::Send_PIV : AskPassphraseDialog::Context::Send_zYODA, true)){
...
bool SendWidget::sendZpiv(QList<SendCoinsRecipient> recipients){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        Q_EMIT message(tr("Spend Zerocoin"), tr("zYODA is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zYODA selector if applicable
    ...
    if (walletModel->sendZpiv(
        ...
        inform(tr("zYODA transaction sent!"));
        ...
        if (receipt.GetStatus() == ZPIV_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zYODA require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zYODA transaction
        ...
        Q_EMIT message("zYODA transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isPIV){
        ...
        if (walletModel->getZerocoinBalance() > 0) {
            ...
            inform(tr("You don't have any zYODA in your balance to select."));
        }
    }
}
