...
#include "qt/yoda/privacywidget.h"
#include "qt/yoda/forms/ui_privacywidget.h"
#include "qt/yoda/qtutils.h"
#include "guiutil.h"
#include "qt/yoda/denomgenerationdialog.h"
#include "qt/yoda/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(YODAGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zYODA anonymizes your YODA by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zYODA or convert back to YODA"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 YODA ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zYODA");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zYODA");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zYODA");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zYODA");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zYODA");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zYODA");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zYODA");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zYODA");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zYODA Movements"));
    ...
    // Only Convert to YODA enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzYODA->setTitleClassAndText("btn-title-grey", "Total 0 zYODA");
    ui->btnTotalzYODA->setSubTitleClassAndText("text-subtitle", "Show denominations of zYODA owned.");
    ui->btnTotalzYODA->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select YODA outputs to mint into zYODA.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zYODA");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzYODA, SIGNAL(clicked()), this, SLOT(onTotalZpivClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zYODA");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of YODA to mint into zYODA"));
    }else{
        btnText = tr("Convert back to YODA");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zYODA to convert back into YODA"));
    }
...
void PrivacyWidget::onTotalZpivClicked(){
    ...
    if(!isVisible){
        ...
        ui->btnTotalzYODA->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzYODA->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zYODA is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zYODA, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zYODA").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zYODA minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZpiv(
        ...
        // Spend succeed
        inform(tr("zYODA converted back to YODA"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::onCoinControlClicked(){
    if(ui->pushRight->isChecked()) {
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any YODA to select."));
        }
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zYODA </b>";
    ...
    ui->btnTotalzYODA->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
