...
#include "qt/yoda/coldstakingwidget.h"
#include "qt/yoda/forms/ui_coldstakingwidget.h"
#include "qt/yoda/qtutils.h"
#include "amount.h"
#include "guiutil.h"
#include "qt/yoda/requestdialog.h"
#include "qt/yoda/tooltipmenu.h"
#include "qt/yoda/furlistrow.h"
...
ColdStakingWidget::ColdStakingWidget(YODAGUI* parent) :
    PWidget(parent),
    ...
    isLoading(false)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can delegate your YODAs, letting a hot node (24/7 online node)\nstake on your behalf, while you keep the keys securely offline."));
    ...
    ui->labelSubtitle2->setText(tr("Delegate or Accept YODA delegation"));
    ...
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", tr("Select YODA outputs to delegate."));

    ui->btnColdStaking->setTitleClassAndText("btn-title-grey", tr("Create Cold Staking Address"));
    ui->btnColdStaking->setSubTitleClassAndText("text-subtitle", tr("Creates an address to receive coin delegations and be able to stake them."));
...
void ColdStakingWidget::updateStakingTotalLabel()
{
    ...
    ui->labelStakingTotal->setText(tr("Total Staking: %1").arg(
            (total == 0) ? "0.00 YODA" : GUIUtil::formatBalance(total, nDisplayUnit))
    );
}
