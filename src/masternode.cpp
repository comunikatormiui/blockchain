...
bool CMasternodeBroadcast::Sign(const CKey& key, const CPubKey& pubKey, const bool fNewSigs)
{
    ...
    LogPrintf("CMasternodeBroadcast::Sign 1: Step 1: fNewSigs = %s \n", fNewSigs);
    if (fNewSigs) {
...
bool CMasternodeBroadcast::Sign(const std::string strSignKey, const bool fNewSigs)
{
    CKey key;
    CPubKey pubkey;
    LogPrintf("CMasternodeBroadcast::Sign 2: Step 1: fNewSigs = %s \n", fNewSigs);
...
bool CMasternodeBroadcast::CheckAndUpdate(int& nDos)
{
    ...
    if (Params().NetworkID() == CBaseChainParams::MAIN) {
        if (addr.GetPort() != 51462) return false;
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 1000 YODA tx -> 1 confirmation
    ...
    bool isLocal = addr.IsRFC1918() && !Params().IsRegTestNet();
