...
#if defined(NDEBUG)
#error "YODA cannot be compiled without assertions."
#endif
...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("%s : zYODA spend tx %s already in block %d",
            ...
            // Reject legacy zYODA mints
            if (!Params().IsRegTestNet() && tx.HasZerocoinMintOutputs())
                return state.Invalid(error("%s : tried to include zYODA mint output in tx %s",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    const int posStagePeriod = 60 * 24 * 30 * 6;
    const int posMultiply = nHeight / posStagePeriod;
    int64_t nSubsidy = 0;

    if (nHeight == 0) {
        nSubsidy = 20000000 * COIN;
    } else if (nHeight > 0 && nHeight <= last_pow_block) {
        nSubsidy = ((Params().GetConsensus().nPremineAmount / COIN )/ last_pow_block) * COIN;
    } else if (nHeight > (posStagePeriod * posMultiply + last_pow_block) && nHeight <= (posStagePeriod * (posMultiply + 1) + last_pow_block) && posMultiply < 10) {
        nSubsidy = (100 - (posMultiply * 10)) * COIN;
    } else {
        nSubsidy = 10 * COIN;
    }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZPIVStake)
{
    ...
    if (nHeight <= Params().GetConsensus().height_last_PoW) {
        ...
        //When zYODA is staked, masternode only gets 2 YODA
        ret = 5 * COIN;
        if (isZPIVStake)
            ret = 5 * COIN;
    }
...
{
    return !(invalid_out::ContainsOutPoint(out));
}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from YODA
...
{
    util::ThreadRename("yoda-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    //Track zYODA money supply in the block index
    if (!UpdateZPIVSupply(block, pindex, fJustCheck))
        return state.DoS(100, error("%s: Failed to calculate new zYODA supply for block=%s height=%d", __func__,
...
bool CheckColdStakeFreeOutput(const CTransaction& tx, const int nHeight)
{
    ...
    if (outs >=3 && lastOut.scriptPubKey != tx.vout[outs-2].scriptPubKey) {
        ...
        // cold staking is active much after nPublicZCSpends so GetMasternodePayment is always 3 YODA.
        // TODO: double check this if/when MN rewards change
        if (lastOut.nValue == 10 * COIN)
            return true;
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zYODA spends in this block
        if (tx.HasZerocoinSpendInputs()) {
            for (const CTxIn& txIn : tx.vin) {
                bool isPublicSpend = txIn.IsZerocoinPublicSpend();
                if (txIn.IsZerocoinSpend() || isPublicSpend) {
                    ...
                    if (std::count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zYODA serial %s in block\n Block: %s",
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        if (isBlockFromFork) {
            ...
            // Now that this loop if completed. Check if we have zYODA inputs.
            if(hasZPIVInputs) {
                for (const CTxIn& zPivInput : zYODAInputs) {
...
void static ProcessGetData(CNode* pfrom)
{
    ...
    LogPrintf("%s: Not Found Size = %d\n", __func__,vNotFound.size());
    LOCK(cs_main);
    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        const CInv& inv = *it;
        {
            ...
            if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK) {
                ...
                LogPrintf("ProcessGetData(): Inv: type = %s inv = %s\n", inv.type, inv.ToString());
                ...
                bool pushed = false;
                {
                    ...
                    if (mi != mapRelay.end()) {
                        ...
                        LogPrintf("ProcessGetData(): Send  command\n");
                        pushed = true;
                    }
                }
                ...
                if (!pushed && inv.type == MSG_TX) {
                    ...
                    if (mempool.lookup(inv.hash, tx)) {
                        ...
                        LogPrintf("ProcessGetData(): Send command: tx \n");
                        pushed = true;
                    }
                }
                ...
                if (!pushed && inv.type == MSG_TXLOCK_VOTE) {
                    if (mapTxLockVote.count(inv.hash)) {
                        ...
                        LogPrintf("ProcessGetData(): Send command: txlvote \n");
                ...
                if (!pushed && inv.type == MSG_TXLOCK_REQUEST) {
                    if (mapTxLockReq.count(inv.hash)) {
                        ...
                        LogPrintf("ProcessGetData(): Send command: ix \n");
                ...
                if (!pushed && inv.type == MSG_SPORK) {
                    if (mapSporks.count(inv.hash)) {
                        ...
                        LogPrintf("ProcessGetData(): Send command: spork \n");
                        pushed = true;
                    }
                }
                if (!pushed && inv.type == MSG_MASTERNODE_WINNER) {
                    if (masternodePayments.mapMasternodePayeeVotes.count(inv.hash)) {
                        ...
                        LogPrintf("ProcessGetData(): Send command: mnw \n");
                ...
                if (!pushed && inv.type == MSG_BUDGET_VOTE) {
                    if (budget.mapSeenMasternodeBudgetVotes.count(inv.hash)) {
                        ...
                        LogPrintf("ProcessGetData(): Send command: mvote \n");
                        pushed = true;
                    }
                }
                ...
                if (!pushed && inv.type == MSG_BUDGET_PROPOSAL) {
                    if (budget.mapSeenMasternodeBudgetProposals.count(inv.hash)) {
                        ...
                        LogPrintf("ProcessGetData(): Send command: mprop \n");
                        pushed = true;
                    }
                }
                ...
                if (!pushed && inv.type == MSG_BUDGET_FINALIZED_VOTE) {
                    if (budget.mapSeenFinalizedBudgetVotes.count(inv.hash)) {
                        ...
                        LogPrintf("ProcessGetData(): Send command: fbvote \n");
                        pushed = true;
                    }
                }
                ...
                if (!pushed && inv.type == MSG_MASTERNODE_ANNOUNCE) {
                    LogPrintf("ProcessGetData(): mnodeman.mapSeenMasternodeBroadcast size = %s \n", mnodeman.mapSeenMasternodeBroadcast.size());
                    if (mnodeman.mapSeenMasternodeBroadcast.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnodeman.mapSeenMasternodeBroadcast[inv.hash];
                        pfrom->PushMessage("mnb", ss);
                        LogPrintf("ProcessGetData(): Send command: mnb \n");
                        pushed = true;
                    }
                }
                ...
                if (!pushed && inv.type == MSG_MASTERNODE_PING) {
                    if (mnodeman.mapSeenMasternodePing.count(inv.hash)) {
                        ...
                        LogPrintf("ProcessGetData(): Send command: mnp \n");
                        pushed = true;
                    }
                }
                ...
                if (!pushed && inv.type == MSG_DSTX) {
                    if (mapObfuscationBroadcastTxes.count(inv.hash)) {
                        ...
                        LogPrintf("ProcessGetData(): Send command: dstx \n");
                        pushed = true;
                    }
                }
...
{
    LogPrint("net", "received: %s (%u bytes) peer=%d, addr=%s\n", SanitizeString(strCommand), vRecv.size(), pfrom->id, pfrom->addrName);
    ...
    if (strCommand == "version") {
        ...
        // YODA: We use certain sporks during IBD, so check to see if they are
