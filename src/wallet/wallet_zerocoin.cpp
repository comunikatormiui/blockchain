// Copyright (c) 2020 The YODA developers
        ...
        std::list<std::pair<CBitcoinAddress*,CAmount>> addressesTo, CBitcoinAddress* changeAddress)
{
    ...
    receipt.SetStatus("Spend Successful", ZPIV_SPEND_OKAY);  // When we reach this point spending zYODA was successful
        ...
        CBitcoinAddress* changeAddress)
{
    ...
    if (nValue < 1) {
        receipt.SetStatus(_("Value is below the smallest available denomination (= 1) of zYODA"), nStatus);
        return false;
    }
    ...
    const int nMaxSpends = Params().Zerocoin_MaxPublicSpendsPerTransaction(); // Maximum possible spends for one zYODA public spend transaction
    std::vector<CMintMeta> vMintsToFetch;
    if (vSelectedMints.empty()) {
        //  All of the zYODA used in the public coin spend are mature by default (everything is public now.. no need to wait for any accumulation)
        ...
        // Select the zYODA mints to use in this spend
    ...
    wtxNew.BindWallet(this);
    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            for (std::pair<CBitcoinAddress*,CAmount> pair : addressesTo){
                ...
                //add output to yoda address to the transaction (the actual primary spend taking place)
                // TODO: check value?
...
{
    if (fMatureOnly) {
        // This code is not removed just for when we back to use zYODA in the future, for now it's useless,
...
void CWallet::setZWallet(CzYODAWallet* zwallet)
{
    zwalletMain = zwallet;
    zpivTracker = std::unique_ptr<CzYODATracker>(new CzYODATracker(strWalletFile));
}

CzYODAWallet* CWallet::getZWallet()
{
    return zwalletMain;
}
