...
{
    LogPrintf("Start CMasternodeSync::GetNextAsset(): %s\n", RequestedMasternodeAssets);
...
void CMasternodeSync::Process()
{
    ...
    for (CNode* pnode : vNodes) {
        ...
        LogPrintf("CMasternodeSync::Process: Sync node: %s version %s \n", pnode->addrName, pnode->nVersion);
        //set to synced
        ...
        if (pnode->nVersion >= masternodePayments.GetMinMasternodePaymentsProto()) {
            if (RequestedMasternodeAssets == MASTERNODE_SYNC_LIST) {
                ...
                LogPrint("masternode", "CMasternodeSync::Process() Step 1\n");
                ...
                if (lastMasternodeList == 0 &&
                    ...
                    LogPrint("masternode", "CMasternodeSync::Process() Step 1-1\n");
                    return;
                }
                LogPrint("masternode", "CMasternodeSync::Process() Step 2\n");
                if (RequestedMasternodeAttempt >= MASTERNODE_SYNC_THRESHOLD * 3) return;
                LogPrint("masternode", "CMasternodeSync::Process() Step 3\n");
                mnodeman.DsegUpdate(pnode);
                RequestedMasternodeAttempt++;
                return;
            }
