...
//
// Bootup the Masternode, look for a 10000 Yoda input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    LogPrintf("CActiveMasternode::ManageStatus() 1 Status = %s\n", status);
    ...
    LogPrintf("CActiveMasternode::ManageStatus() 2 Status = %s\n", status);
    if (fDebug) LogPrintf("CActiveMasternode::ManageStatus() - Step 1\n");
    if (status == ACTIVE_MASTERNODE_SYNC_IN_PROCESS) status = ACTIVE_MASTERNODE_INITIAL;
    LogPrintf("CActiveMasternode::ManageStatus() 3 Status = %s\n", status);
    ...
    if (fDebug) LogPrintf("CActiveMasternode::ManageStatus() - Step 2\n");

    if (status != ACTIVE_MASTERNODE_STARTED) {
        // Set defaults
        status = ACTIVE_MASTERNODE_NOT_CAPABLE;
        notCapableReason = "";
        if (fDebug) LogPrintf("CActiveMasternode::ManageStatus() - Step 3\n");
        if (pwalletMain->IsLocked()) {
            notCapableReason = "Wallet is locked.";
            LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", notCapableReason);
            return;
        }
        if (fDebug) LogPrintf("CActiveMasternode::ManageStatus() - Step 4\n");
        if (pwalletMain->GetBalance() == 0) {
            notCapableReason = "Hot node, waiting for remote activation.";
            LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", notCapableReason);
            return;
        }
        if (fDebug) LogPrintf("CActiveMasternode::ManageStatus() - Step 5\n");
        ...
        if (fDebug) LogPrintf("CActiveMasternode::ManageStatus() - Step 6\n");
        // The service needs the correct default port to work properly
        if(!CMasternodeBroadcast::CheckDefaultPort(strMasterNodeAddr, errorMessage, "CActiveMasternode::ManageStatus()"))
            return;

        if (fDebug) LogPrintf("CActiveMasternode::ManageStatus() - Step 7\n");
        ...
        pnode->Release();
        if (fDebug) LogPrintf("CActiveMasternode::ManageStatus() - Step 8\n");
        ...
        if (GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            if (fDebug) LogPrintf("CActiveMasternode::ManageStatus() - Step 9\n");
            ...
            if (fDebug) LogPrintf("CActiveMasternode::ManageStatus() - Step 10\n");
            ...
            if (fDebug) LogPrintf("CActiveMasternode::ManageStatus() - Step 11\n");
            //send to all peers
            LogPrintf("CActiveMasternode::ManageStatus() - Relay broadcast vin = %s\n", vin.ToString());
            mnb.Relay();

            LogPrintf("CActiveMasternode::ManageStatus():mapSeenMasternodePing = %s, mapSeenMasternodeBroadcast = %s \n", mnodeman.mapSeenMasternodePing.size(), mnodeman.mapSeenMasternodeBroadcast.size());
    ...
    if (fDebug) LogPrintf("CActiveMasternode::ManageStatus() - Step Last\n");
    //send to all peers
    if (!SendMasternodePing(errorMessage)) {
        LogPrintf("CActiveMasternode::ManageStatus() - Error on Ping: %s\n", errorMessage);
    }
    if (fDebug) LogPrintf("CActiveMasternode::ManageStatus() - End\n");
}
...
bool CActiveMasternode::SendMasternodePing(std::string& errorMessage)
{
    ...
    if (pmn != NULL) {
        LogPrintf("CActiveMasternode::SendMasternodePing() - not NULL\n");
...
bool CActiveMasternode::CreateBroadcast(CTxIn vin, CService service, CKey keyCollateralAddress, CPubKey pubKeyCollateralAddress, CKey keyMasternode, CPubKey pubKeyMasternode, std::string& errorMessage, CMasternodeBroadcast &mnb)
{
    ...
    LogPrintf("%s: CMasternodeBroadcast Step 1\n",__func__);
