...

    {
        ...
        for (std::map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                // zYODA spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent zYODA spends in this block or tx
            if (tx.HasZerocoinSpendInputs()) {
                ...
                //This zYODA serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
        ...
        if (!fProofOfStake) {
            ...
            LogPrintf("%s: txNew.vin[0].scriptSig size = %s \n", __func__,txNew.vin[0].scriptSig.size());
        }
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("YODAMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        return error("YODAMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("YODAMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("yoda-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            while (vNodes.empty() || pwallet->IsLocked() || !fStakeableCoins || masternodeSync.NotCompleted()) {
                MilliSleep(5000);
        ...
        LogPrintf("Running YODAMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("YODAMiner exception");
    } catch (...) {
        LogPrintf("YODAMiner exception");
    }

    LogPrintf("YODAMiner exiting\n");
}
