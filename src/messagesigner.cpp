...
{
    std::string strError = "";
    LogPrintf("CSignedMessage::Sign: Begin, fNewSigs = %s\n", fNewSigs);
    if (fNewSigs) {

        nMessVersion = MessageVersion::MESS_VER_HASH;
        uint256 hash = GetSignatureHash();
        LogPrintf("CSignedMessage::Sign: SignHash\n");
        if(!CHashSigner::SignHash(hash, key, vchSig)) {
            return error("%s : SignHash() failed", __func__);
        }
        LogPrintf("CSignedMessage::Sign: VerifyHash\n");
    ...
    LogPrintf("CSignedMessage::Sign: End\n");
    return true;
}
