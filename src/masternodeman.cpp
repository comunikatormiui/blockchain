...
void CMasternodeMan::AskForMN(CNode* pnode, CTxIn& vin)
{
    ...
    LogPrintf("%s: Asking node for missing entry, vin: %s\n", __func__, vin.prevout.hash.ToString());
...
CMasternode* CMasternodeMan::Find(const CTxIn& vin)
{
    ...
    for (CMasternode& mn : vMasternodes) {
        if (fDebug) LogPrintf("CMasternodeMan::Find: - Iterate MN\n");
...
void CMasternodeMan::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    LogPrintf("CMasternodeMan::ProcessMessage() : Start: cmd = %s\n", strCommand);
    LOCK(cs_process_message);

    if (strCommand == "mnb") { //Masternode Broadcast
        CMasternodeBroadcast mnb;
        vRecv >> mnb;
        LogPrintf("CMasternodeMan::ProcessMessage() : mnb\n");
        ...
        CTxIn vin;
        vRecv >> vin;
        LogPrintf("CMasternodeMan::ProcessMessage() : dseg\n");
    ...
    // Light version for OLD MASSTERNODES - fake pings, no self-activation
        ...
        LogPrintf("CMasternodeMan::ProcessMessage() : dsee\n");
        ...
        if (Params().NetworkID() == CBaseChainParams::MAIN) {
            if (addr.GetPort() != 51462) return;
        } else if (addr.GetPort() == 51462)
            return;
        ...
        if (fAcceptable) {
            ...
            // should be at least not earlier than block when 1000 YODA tx got MASTERNODE_MIN_CONFIRMATIONS
            uint256 hashBlock = 0;
            CTransaction tx2;
            GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
            BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
            if (mi != mapBlockIndex.end() && (*mi).second) {
                CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 10000 YODA tx -> 1 confirmation
        ...
        LogPrintf("CMasternodeMan::ProcessMessage() : dseep\n");
        if (sporkManager.IsSporkActive(SPORK_10_MASTERNODE_PAY_UPDATED_NODES)) return;
